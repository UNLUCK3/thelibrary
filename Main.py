...
morals = 0
loop = 0
quotemark = input("please enter the game code provided by the developer for testing purposes: ")
globname = input("NAME: ")
html_escape_table = {
    "&": "&amp;",
    '"': "&quot;",
...
def intro():
    ...
    time.sleep(1.5)
    print("please only use numerical digits, and lowercase type when necessary.")
    time.sleep(3)
    name = input("NAME: ")
    if name.lower() == "bryn":
...
def library1():
    ...
    time.sleep(3)
    print(
        "This library has 3 branches, to my left we have the books, and to my right we have the commons... The third "
        "branch is off limits at the moment.")
    time.sleep(2)
    print(
        "TYPE 1 FOR THE FIRST BRANCH, AND 2 FOR THE SECOND BRANCH, OR DO WHATEVER, YOU'RE THE BOSS AND I'M JUST A "
        "NARRATOR.")
    branch_selection = input("AWAITING INPUT: ")
...
def commons_branch():
    ...
    talkto = input("TYPE YOUR DECISION HERE: ")
    if talkto == "3":
...
def first_person1():
    ...
    choice_person1 = input("WAITING FOR NUMBER:")
    if choice_person1 == "1":
...
# grassy fields, answering no to inspector question
def door1():
    global morals
    print("null")
